...
\noindent %???
{14.} %\textit{(solved)}
Assume that the real intervals $I_1,\dots,I_k$ have $2\cdot k$ different ends that are chousen randomly from $\{1,2,\dots,2\cdot k\}$.
...
\noindent %
{15.} %\textit{(solved)}
Understand the proof of Erd\H{o}s--Lov\'{a}sz theorem (see the references for Theorem 2.1.5 in ``Pearls'' or find a proof elsewhere).

\end{document}

\ 

\noindent {14.}\textit{(solved)} Show that a graph $G$ with $p$ vertexes is a tree if and only if
