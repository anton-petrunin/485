...
(The score depends on the types of mistakes).
...
\noindent \sout{14.} \textit{(solved)}  Solve 2.13 from ``Extra pearls''.
...
\noindent \sout{16.} In a group of people, for some fixed $s$ and any $k$,
...
\noindent {18.} Show that a graph $G$ with $p$ vertexes is a tree if and only if
